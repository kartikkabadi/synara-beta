...
### Safe to Synchronize
- **Settings (`userdata/settings.json`)**: Synchronizes provider options, model choices, UI themes, custom instructions, and typography. Automatically sanitized: inline passwords and server tokens are stripped so Beta mints independent credentials.
...
3. Automatically creates pre-sync timestamped snapshot backups in `userdata/backups/pre-sync-<timestamp>` before modifying any Beta files, allowing instant undo.
