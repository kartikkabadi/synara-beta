...
if ! command -v ssh-keygen >/dev/null 2>&1; then
  echo "install-linux.sh: ssh-keygen is required to verify the release signature. Install the OpenSSH client (apt install openssh-client, dnf install openssh-clients) and retry." >&2
  exit 1
fi
